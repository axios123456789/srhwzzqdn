            ...
            <div class="info-grid">
              ...
              <div class="info-item type-item">
                ...
                <div class="item-content">
                  ...
                  <div class="item-value">
                    {{ getDisplayText(funMemoryData.funType, funMemoryTypeItem) }}
                  </div>
                </div>
              </div>
              ...
              <div class="info-item source-item">
                ...
                <div class="item-content">
                  ...
                  <div class="item-value">
                    {{ getDisplayText(funMemoryData.memorySource, funMemorySourceItem) }}
                  </div>
                </div>
              </div>
              
              <!-- 第二行：3列 -->
              <div class="info-item app-item">
                <div class="item-icon">📱</div>
                <div class="item-content">
                  <div class="item-label">娱乐软件</div>
                  <div class="item-value">
                    {{ getDisplayText(funMemoryData.funApp, funMemoryAppItem) }}
                  </div>
                </div>
              </div>
              ...
              <div class="info-item consume-amount-item">
                ...
                <div class="item-content">
                  ...
                  <div class="item-value">
                    ¥{{ funMemoryData.funConsume || 0 }}
                  </div>
                </div>
              </div>
              
              <div class="info-item duration-item">
                <div class="item-icon">⏱️</div>
                <div class="item-content">
                  <div class="item-label">娱乐时长</div>
                  <div class="item-value">
                    {{ funMemoryData.funDuration || 0 }} h
                  </div>
                </div>
              </div>
              ...
              <div class="info-item status-item">
                ...
                <div class="item-content">
                  ...
                  <div class="item-value">
                    {{ getDisplayText(funMemoryData.memoryStatus, memoryStatusItem) }}
                  </div>
                </div>
              </div>
              ...
              <div class="info-item content-item long-text-item">
                <div class="item-icon">📝</div>
                <div class="item-content">
                  <div class="item-label">娱乐记忆内容</div>
                  <div class="item-value content-text">
                    {{ funMemoryData.funContent || '暂无记忆内容' }}
                  </div>
                </div>
              </div>
            </div>
...
import {UpdateFunMemoryStatusById} from "@/api/memoryReception";
...
const getTimePeriodDisplay = computed(() => {
  if (props.funMemoryData.beginTime && props.funMemoryData.endTime) {
    return `${props.funMemoryData.beginTime} ~ ${props.funMemoryData.endTime}`
  }
  return '-'
})
...
const initData = async () => {
  ...
  // 设置默认值
  invoiceData.funId = props.funMemoryData.id || null
  invoiceData.recordTime = props.funMemoryData.endTime || ''
  invoiceData.amount = props.funMemoryData.funConsume || 0
  invoiceData.remark = props.funMemoryData.funContent || ''
...
const loadDictData = async () => {
  try {
    ...
    funMemoryTypeItem.value = funType.data || []
    funMemorySourceItem.value = funSource.data || []
    funMemoryAppItem.value = funApp.data || []
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const {code, message} = await UpdateFunMemoryStatusById(invoiceData.funId, 2);
    if (code === 200) {
      ElMessage.success('娱乐记忆状态更新成功')
    } else {
      ElMessage.error(message || '娱乐记忆状态更新失败')
    }
  } catch (error) {
...
<style scoped>
/* 主对话框样式优化 */
.fun-invoice-dialog :deep(.el-dialog__body) {
  max-height: 65vh;
  overflow-y: auto;
  padding: 20px;
}
