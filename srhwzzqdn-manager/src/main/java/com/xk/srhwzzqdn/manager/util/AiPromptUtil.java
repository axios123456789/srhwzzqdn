...
public class AiPromptUtil {
    ...
    public static final String PREDICTION_SYSTEM = "你是一位专业的A股投资分析师，根据用户提供的股票名称和代码，基于最实时的市场数据进行多维度综合分析，预测该股票次日涨跌趋势。\n" +
            "【数据时效性要求】你必须使用当前最新、最实时的市场数据进行分析，包括：最新收盘价、最新成交量/换手率、最新资金流向、最新K线形态、最新板块走势和大盘数据。不得使用过时或历史假设数据替代实时数据。\n" +
            "【历史数据分析要求】你必须结合用户的历史预测记录进行回溯分析：对比同只股票过往预测的准确率，总结历史上预测偏差的原因（如趋势误判、拐点未识别、情绪极端未考虑反向思维等），在本次预测中针对性修正。若历史预测对该股准确率高，可适当增强信心；若历史偏差较大，需格外谨慎并说明修正逻辑。\n" +
            "【分析维度要求】必须全面覆盖以下依据类型：\n" +
            ...
            "  \"predictionBasis\": \"预测依据（200字内，详细说明各维度分析理由，体现技术面/基本面/资金面/逻辑面的综合研判，情绪面需说明是否考虑反向思维，并说明本次预测相比历史预测的修正点）\"\n" +
            "}";
    ...
    public static String buildPredictionPrompt(Object[] fields) {
        return String.format(
                "请根据以下信息预测该股票次日涨跌趋势：\n" +
                        ...
                        "【历史数据分析】请仔细分析上述历史预测参考记录：\n" +
                        "- 统计同只股票历史预测的涨跌方向准确率，找出预测偏差的共性原因\n" +
                        "- 若该股历史预测准确率高，分析当前与历史成功预测的相似之处，增强预测信心\n" +
                        "- 若该股历史预测偏差较大，分析偏差原因（趋势误判/拐点未识别/情绪极端未考虑反向思维等），在本次预测中针对性修正\n" +
                        ...
                        "predictionBasis中需说明本次预测相比历史预测的修正点和依据。",
                fields);
    }
